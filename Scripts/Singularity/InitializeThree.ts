...
export async function main(ns: NS): Promise<void> {
  ...
  const csecMaxRep = Math.max(...csecAugments.map(function (augment: string) {
  ...
  const s12MaxRep = Math.max(...s12Augments.map(function (augment: string) {
    return ns.singularity.getAugmentationRepReq(augment);
  }));
  ...
  const aevMaxRep = Math.max(...aevAugments.map(function (augment: string) {
    return ns.singularity.getAugmentationRepReq(augment);
  }));
