...
async function main(ns) {
    ...
    const weakenRatio1 = 0.004 / 0.05 / hackToGrowRatio / (0.004 / 0.05 / hackToGrowRatio + 1 / hackToGrowRatio);
    const weakenRatio2 = 0.002 / 0.05 / (0.002 / 0.05 + 1);
    const growRatio = 1 / hackToGrowRatio / (0.004 / 0.05 / hackToGrowRatio + 1 / hackToGrowRatio);
    const hackRatio = 1 / (0.002 / 0.05 + 1);
    ...
    while(minSecurity <= (security = ns.getServerSecurityLevel(target))){
        ...
        while((security = ns.getServerSecurityLevel(target)) <= minSecurity + growThreads * 0.004){
            ...
            if (weaken1 && Math.ceil((performance.now() + growExecTime) / initBuffer) % 2 == 1 && weaken1 - growExecTime - performance.now() < initBuffer) {
                weakenQueue1.shift();
                ns.run(grow, growThreads, target, true, stage);
            }
            if (weaken2 && Math.ceil((performance.now() + hackExecTime) / initBuffer) % 2 == 1 && weaken2 - hackExecTime - performance.now() < initBuffer) {
                weakenQueue2.shift();
