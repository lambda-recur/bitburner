...
async function main(ns) {
    ...
    const targetServers = Array.from(servers).filter(function(target) {
        return !pservers.concat([
            "home"
        ]).includes(target);
    });
    ...
    const targets = Array.from(servers).filter(function(target) {
    ...
    const targetsSorted = targets.sort(function(a, b) {
        ...
        return ns.getServerRequiredHackingLevel(target) < ns.getHackingLevel() / 3;
    }).slice(1);
    for (const pserver of pserversSorted){
        ...
        ns.exec(script1, pserver, 1, "BatchHack.js", targetsSorted[pserversSorted.indexOf(pserver)]);
    }
    for (const target of targetServers){
        ...
        if (hLevel >= sRHL) {
            ...
            ns.killall(target);
            const threads = Math.floor(ns.getServerMaxRam(target) / ns.getScriptRam(script2));
            if (threads > 0) {
                await ns.exec(script2, target, Math.floor(ns.getServerMaxRam(target) / ns.getScriptRam(script2)));
            }
        }
    }
}
